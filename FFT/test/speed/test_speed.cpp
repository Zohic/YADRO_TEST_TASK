...
const cstr method_names[6] = {"dft", "idft", "subdivided dft", "subdivided idft", "fft", "ifft"};
...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < pts_cnt; i++) {
		points[i].real(-5 + (number_t)rand() / RAND_MAX * 10);
		points[i].imag(-5 + (number_t)rand() / RAND_MAX * 10);
	}
	...
	for (int i = 0; i < 6; i++)
		printf("average time %s = %10.20f ms\n", method_names[i], (double)total_time[i] / tests);
	std::getchar();
}
