...
При разделении преобразование фурье с длиной преобразования $N$ на две части $N = N_1 * N_2$ асимптотическая сложность уменьшается с $O(N^2)$ до $O(N_1N_2)$
 ...
 <br/>  
 Данный метод уже позволяет ускорить преобразования длиной кратной 2, 3, 5

 3. ### Дискретное преобразование Фурье с рекурсивным разделением на множители (Вариант алгоритма Кули-Тьюки)<br/>
   Длина преобразования рекурсивно разбивается на подмножители 2, 3, 5, и др. Каждое разбиние уменьшает количество необходимых инструкций в итоге<br/>
...
## Что добавлено
- Система сборки CMake 3.18 + Ninja и скрипты для простой сборки. Нужнен компилятор Clang или GCC. (Указаный в PATH)
- Улучшен рекурсивный алгоритм, так что он:
...
- в класс FFT добавлен подкласс thread_queue<num_threads> который является простым специализированным пулом потоков. Потоки выделяются заранее и ожидают команд для вычислений dft по строкам матриц. Количество выделенных потоков задается как аргумент шаблона, задан в [fft.hpp](./include/fft.hpp) ``NUM_DFT_THREADS``
